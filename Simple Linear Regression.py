...
def train_test_split(x,y,train_size,test_size):
    assert sum([train_ratio, test_ratio])==1.0,
    assert X.shape[0]==len(y),

    ind_train = int(round(X.shape[0]*train_ratio))
    ind_test = int(round(X.shape[0]*(test_ratio)))

    X_train = x[:ind_train]
    y_train = x[ind_test:]

    X_validation = y[:ind_train]
    Y_validation = y[ind_test:]

    return X_train, X_validation, Y_train,Y_validation
X_train, X_validation, Y_train, Y_validation=train_test_split(X, Y, train_ratio, test_ratio)
